...
namespace DataAccess.Repositories.Repo {
    internal class CartRepository : ICartRepository {
        ...
        public void AddCartItem(int UserId, int ItemId, int amount, string? note) {
            //Check Null
            ...
            try {
                var outs = context.Database.SqlQuery<int>($"exec insert_Cart @uid = {UserId} ,@iid = {ItemId} ,@add = {amount} , @note = {note}").ToList();
                if (outs.Count < 1) {
